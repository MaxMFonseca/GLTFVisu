...
describe('default model adapter', () => {
  it('fetches the configured URL and constructs a binary glTF file', async () => {
    ...
    await expect(fetchDefaultModel(
      { url: '/assets/suzanne.glb', fileName: 'Suzanne.glb' },
      fetcher,
    )).resolves.toMatchObject({ name: 'Suzanne.glb', type: 'model/gltf-binary' })
    expect(fetcher).toHaveBeenCalledWith('/assets/suzanne.glb')
  })
  ...
  it('rejects an empty default model filename without fetching', async () => {
    ...
    await expect(fetchDefaultModel(
      { url: '/assets/suzanne.glb', fileName: '  ' },
      fetcher,
  ...
  it('downloads the default model through the browser fetch API', async () => {
    ...
    const blob = await fetchDefaultModelBlob('/assets/suzanne.glb', fetcher)
